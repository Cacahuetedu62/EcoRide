...
if (isset($_SESSION['utilisateur']) && isset($_SESSION['utilisateur']['id'])) {
    ...
    $sql = "SELECT h.id, h.date_debut_reel, h.date_fin_reel, h.date_enregistrement, t.lieu_depart, t.lieu_arrive, t.prix_personnes,
            t.date_depart, t.heure_depart, t.date_arrive, t.heure_arrive,
            ...
            FROM historique h
            JOIN trajets t ON h.trajet_id = t.id
            LEFT JOIN passagers p ON h.id = p.reservation_id
            JOIN trajet_utilisateur tu ON h.trajet_id = tu.trajet_id AND tu.utilisateur_id = :utilisateur_id
            ...
            if (!empty($trajets_conducteur)): ?>
                <?php foreach ($trajets_conducteur as $trajet): ?>
                    <div class="vehicule givre">
                        ...
                        <p>Prix total : <?= htmlspecialchars($trajet['prix_total'], ENT_QUOTES, 'UTF-8') ?> €</p>
                        <br>
                        <p>Vous avez participé à ce trajet avec : <?= htmlspecialchars($trajet['participants'], ENT_QUOTES, 'UTF-8') ?></p>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
